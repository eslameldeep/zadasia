...
@props(['options', 'selected' ,
 'labelClass' => $labelClass ?? 'text-lightblue'])
...
@section('input_group_item')
    ...
    <select id="{{ $id }}" name="{{ $name }}" {{ $attributes->merge(['class' => $makeItemClass()]) }}>
        ...
        @foreach ($options ?? [] as $val => $option_name)
            @php
            if (old(str_replace('[]', '', $name)) ){
                old(str_replace('[]', '', $name)) ;
            }
                $oldValue = is_array(old(str_replace('[]', '', $name))) ? old(str_replace('[]', '', $name)) : [old(str_replace('[]', '', $name))];
                $isSelected = in_array((string) $val, $oldValue) || $selected->contains($option_name);
            @endphp
            <option {{ $isSelected ? 'selected' : '' }} value="{{ $val }}">{{ $option_name }}</option>
        @endforeach
...
@once
    @push('css')
        <style type="text/css">
            ...
            .select2-selection--multiple { background : rgba(0,0,0,0) !important    }
